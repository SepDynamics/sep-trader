...
bool WeeklyDataFetcher::initializeOandaAPI() {
    ...
    if (!api_key || !account_id || !environment) {
        std::cerr << "❌ CRITICAL ERROR: Missing OANDA credentials!" << std::endl;
        std::cerr << "Required environment variables:" << std::endl;
        std::cerr << "- OANDA_API_KEY" << (api_key ? " ✅" : " ❌") << std::endl;
        std::cerr << "- OANDA_ACCOUNT_ID" << (account_id ? " ✅" : " ❌") << std::endl;
        std::cerr << "- OANDA_ENVIRONMENT" << (environment ? " ✅" : " ❌") << std::endl;
        std::cerr << "System cannot proceed without real market data!" << std::endl;
        return false;
    }
...
DataFetchResult WeeklyDataFetcher::fetchInstrument(const std::string& instrument) {
    ...
    if (config_.oanda_api_key.empty() || config_.oanda_account_id.empty()) {
        result.error_message = "OANDA credentials not configured - refusing to use fake data!";
        std::cerr << "❌ FATAL: System configured with fake data simulation!" << std::endl;
        std::cerr << "❌ This compromises all training accuracy!" << std::endl;
        return result;
    }
