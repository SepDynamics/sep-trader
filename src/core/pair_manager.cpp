...
namespace sep::core
{
    ...
    PairManager::~PairManager() { saveState(); }
                                  ...
                                  double accuracy)
    {
        ...
        if (it->second->status == PairStatus::UNTRAINED ||
            it->second->status == PairStatus::TRAINING)
        {
            PairStatus old_status = it->second->status;
            it->second->status = PairStatus::READY;
    ...
    void PairManager::notifyStateChange(const std::string& symbol, PairStatus old_status,
                                        PairStatus new_status)
    {
        std::lock_guard<std::mutex> lock(callbacks_mutex_);
    ...
    void PairManager::initializeDefaultPairs()
    {
        ...
        std::vector<std::string> default_pairs = {"EUR_USD", "GBP_USD", "USD_JPY", "AUD_USD",
                                                  "USD_CAD", "USD_CHF", "EUR_GBP", "EUR_JPY",
                                                  "EUR_AUD", "GBP_JPY", "AUD_JPY", "NZD_USD"};
        ...
        try
        {
            ...
            if (json.contains("pairs") && json["pairs"].is_object())
            {
                for (auto& [symbol, pair_data] : json["pairs"].items())
                {
                    auto pair_info = std::make_unique<PairInfo>();
                    pair_info->symbol = symbol;
                    pair_info->status = stringToStatus(pair_data.value("status", "UNTRAINED"));
                    pair_info->enabled = pair_data.value("enabled", false);
                    pair_info->accuracy = pair_data.value("accuracy", 0.0);
                    pair_info->model_path =
                        (pair_data.contains("model_path") && !pair_data["model_path"].is_null())
                            ? pair_data["model_path"].get<std::string>()
                            : std::string();
                    pair_info->error_message = (pair_data.contains("error_message") &&
                                                !pair_data["error_message"].is_null())
                                                   ? pair_data["error_message"].get<std::string>()
                                                   : std::string();

                    auto last_updated =
                        pair_data.contains("last_updated") && !pair_data["last_updated"].is_null()
                            ? pair_data["last_updated"].get<long long>()
                            : 0LL;
                    if (last_updated != 0)
                    {
                        pair_info->last_updated =
                            std::chrono::system_clock::from_time_t(last_updated);
                    }
                    else
                    {
                        pair_info->last_updated = std::chrono::system_clock::now();
    ...
    std::string statusToString(sep::core::PairStatus status)
    {
        switch (status)
        {
            case sep::core::PairStatus::UNTRAINED:
                return "UNTRAINED";
            case sep::core::PairStatus::TRAINING:
                return "TRAINING";
            case sep::core::PairStatus::READY:
                return "READY";
            case sep::core::PairStatus::TRADING:
                return "TRADING";
            case sep::core::PairStatus::DISABLED:
                return "DISABLED";
            case sep::core::PairStatus::ERROR:
                return "ERROR";
            default:
                return "UNKNOWN";
        }
