...
struct TrainingResultsSummary {
    ...
    char stopping_reason[128];  // "convergence", "max_iterations", "accuracy_target", "manual",
                                // "error"
...
class QuantumTrainingCoordinator {
  public:
    ...
    bool startIncrementalTraining(const char* pair_symbol, const char* base_model_path,
                                  char* session_id_out);
    ...
    uint32_t getAllActiveSessionsStatus(TrainingSessionStatus* statuses_out,
    ...
    uint32_t getAvailableModelVersions(const char* pair_symbol, char versions_out[][64],
                                       uint32_t max_versions) const;
    bool compareModelVersions(const char* pair_symbol, const char** versions,
                              uint32_t version_count, TrainingResultsSummary* comparison_out) const;
    ...
    uint32_t analyzeTrainingBottlenecks(char bottleneck_names_out[][64],
    ...
    bool generateTrainingReport(const char* session_id, char* report_content_out,
