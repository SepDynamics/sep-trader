...
std::unordered_map<std::string, ConfigValue> EngineConfig::get_category_config(
    ConfigCategory category) const {
...
void EngineConfig::initialize_param_definitions() {
    ...
    param_definitions_["quantum.coherence_threshold"] =
        ConfigParam("quantum.coherence_threshold", ConfigCategory::QUANTUM, 0.5, 0.0, 1.0,
                    "Threshold for quantum coherence detection");

    param_definitions_["quantum.collapse_threshold"] =
        ConfigParam("quantum.collapse_threshold", ConfigCategory::QUANTUM, 0.6, 0.0, 1.0,
                    "Threshold for quantum state collapse");

    param_definitions_["quantum.enable_qfh"] =
        ConfigParam("quantum.enable_qfh", ConfigCategory::QUANTUM, true,
                    "Enable Quantum Field Harmonics analysis");

    // CUDA/GPU parameters
    param_definitions_["cuda.device_id"] =
        ConfigParam("cuda.device_id", ConfigCategory::CUDA, 0, 0, 8, "CUDA device ID to use", true);

    param_definitions_["cuda.enable_gpu"] =
        ConfigParam("cuda.enable_gpu", ConfigCategory::CUDA, true, "Enable GPU acceleration", true);

    param_definitions_["cuda.memory_pool_size_mb"] =
        ConfigParam("cuda.memory_pool_size_mb", ConfigCategory::CUDA, 256, 64, 8192,
                    "GPU memory pool size in megabytes", true);

    // Memory management parameters
    param_definitions_["memory.enable_pattern_cache"] =
        ConfigParam("memory.enable_pattern_cache", ConfigCategory::MEMORY, true,
                    "Enable pattern result caching");

    param_definitions_["memory.cache_size"] =
        ConfigParam("memory.cache_size", ConfigCategory::MEMORY, 1000, 100, 10000,
                    "Maximum number of patterns in cache");

    param_definitions_["memory.cache_ttl_minutes"] =
        ConfigParam("memory.cache_ttl_minutes", ConfigCategory::MEMORY, 60, 1, 1440,
                    "Cache time-to-live in minutes");

    // Batch processing parameters
    param_definitions_["batch.default_max_threads"] =
        ConfigParam("batch.default_max_threads", ConfigCategory::BATCH,
                    static_cast<int>(std::thread::hardware_concurrency()), 1, 64,
                    "Default maximum threads for batch processing");

    param_definitions_["batch.default_batch_size"] =
        ConfigParam("batch.default_batch_size", ConfigCategory::BATCH, 100, 1, 10000,
                    "Default batch size for processing");

    param_definitions_["batch.default_timeout_seconds"] =
        ConfigParam("batch.default_timeout_seconds", ConfigCategory::BATCH, 30.0, 1.0, 300.0,
                    "Default timeout for batch operations in seconds");

    // Streaming parameters
    param_definitions_["streaming.default_buffer_size"] =
        ConfigParam("streaming.default_buffer_size", ConfigCategory::STREAMING, 1000, 100, 100000,
                    "Default buffer size for streaming data");

    param_definitions_["streaming.default_sample_rate_ms"] =
        ConfigParam("streaming.default_sample_rate_ms", ConfigCategory::STREAMING, 100, 10, 10000,
                    "Default sampling rate in milliseconds");

    // Performance parameters
    param_definitions_["performance.enable_optimizations"] =
        ConfigParam("performance.enable_optimizations", ConfigCategory::PERFORMANCE, true,
                    "Enable performance optimizations");

    param_definitions_["performance.parallel_pattern_analysis"] =
        ConfigParam("performance.parallel_pattern_analysis", ConfigCategory::PERFORMANCE, true,
                    "Enable parallel pattern analysis");

    // Debug parameters
    param_definitions_["debug.enable_logging"] =
        ConfigParam("debug.enable_logging", ConfigCategory::DEBUG, true, "Enable debug logging");

    param_definitions_["debug.log_level"] =
        ConfigParam("debug.log_level", ConfigCategory::DEBUG, 2, 0, 4,
                    "Log level (0=off, 1=error, 2=warning, 3=info, 4=debug)");

    param_definitions_["debug.enable_profiling"] = ConfigParam(
        "debug.enable_profiling", ConfigCategory::DEBUG, false, "Enable performance profiling");

    // DSL Interpreter parameters
    param_definitions_["dsl.for_loop_type_error"] =
        ConfigParam("dsl.for_loop_type_error", ConfigCategory::DEBUG,
                    std::string("For loop iterable must be an array"),
                    "Error message for invalid for loop iterable type");

    param_definitions_["dsl.array_bounds_error_template"] =
        ConfigParam("dsl.array_bounds_error_template", ConfigCategory::DEBUG,
                    std::string("Array index {index} out of bounds (size: {size})"),
                    "Error message template for array index out of bounds. Use {index} and {size} "
                    "as placeholders.");

    param_definitions_["dsl.vector_bounds_error_template"] =
        ConfigParam("dsl.vector_bounds_error_template", ConfigCategory::DEBUG,
                    std::string("Vector component index {index} out of bounds (size: {size})"),
                    "Error message template for vector component index out of bounds. Use {index} "
                    "and {size} as placeholders.");

    param_definitions_["dsl.vector_component_names"] =
        ConfigParam("dsl.vector_component_names", ConfigCategory::DEBUG, std::string("x,y,z,w"),
                    "Comma-separated list of valid vector component names");

    param_definitions_["dsl.function_missing_arg_default"] = ConfigParam(
        "dsl.function_missing_arg_default", ConfigCategory::DEBUG, std::string("null"),
        "Default value for missing function arguments (null, 0, false, or empty string)");

    param_definitions_["dsl.if_expr_unknown_type_default"] =
        ConfigParam("dsl.if_expr_unknown_type_default", ConfigCategory::DEBUG, false,
                    "Default boolean value for unknown types in if expressions");
}

bool EngineConfig::is_value_in_range(const ConfigValue& value, const ConfigValue& min_val,
                                     const ConfigValue& max_val) const {
    return std::visit(
        [](const auto& v, const auto& min_v, const auto& max_v) -> bool {
