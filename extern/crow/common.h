...
// Check if crow_isolation.h has already been included
#ifndef SEP_CROW_ISOLATION_INCLUDED
...
// Define CROW_LIKELY if not already defined
#ifndef CROW_LIKELY
...
#endif // SEP_CROW_ISOLATION_INCLUDED
...
constexpr const char* method_strings[] = {
  ...
  "COPY",       "LOCK",       "MKCOL",     "MOVE",        "PROPFIND", "PROPPATCH",
  "SEARCH",     "UNLOCK",     "BIND",      "REBIND",      "UNBIND",   "ACL",
...
struct routing_handle_result {
  ...
  routing_handle_result() : rule_index(0), method(HTTPMethod::Get) {}

  routing_handle_result(uint16_t rule_index_, std::vector<uint16_t> blueprint_indices_,
                       routing_params r_params_)
      : rule_index(rule_index_), blueprint_indices(blueprint_indices_), r_params(r_params_), method(HTTPMethod::Get) {}
...
inline crow::HTTPMethod method_from_string(const char* str, sep::crow::error::Code* ec = nullptr)
{
    return crow::black_magic::is_equ_p(str, "GET", 3)    ? crow::HTTPMethod::Get :
           ...
           (ec ? *ec = sep::crow::error::Code::InvalidMethod : sep::crow::error::Code::None,
            crow::HTTPMethod::Get);
}

inline crow::HTTPMethod operator""_method(const char* str, size_t /*len*/)
{
