...
class BaseRule {
 public:
  ...
  virtual void handle_upgrade(const request& req , response& res,
                              SocketAdaptor&&) {
    (void)req;
    res = response(404);
    res.end();
...
struct Wrapped {
  ...
  void operator()(request& req, response& res, const routing_params& params) {
    detail::routing_handler_call_helper::call<
        ...
        detail::routing_handler_call_helper::call_params<decltype(handler_)>{handler_, params, req,
                                                                             res});
  }
};
 ...
 public:
  ...
  operator()(Func&& f) {
    static_assert(!std::is_same<void, decltype(f())>::value,
                  ...
                  "crow::response, crow::returnable");

    handler_ = ([f = std::forward<Func>(f)](const request&, response& res) {
      res = response(f());
      res.end();
    });
  }
  ...
  template <typename Func>
  typename std::enable_if<!black_magic::CallHelper<Func, black_magic::S<>>::value &&
  ...
  operator()(Func&& f) {
    ...
    handler_ = ([f = std::forward<Func>(f)](const request& req, response& res) {
      res = response(f(req));
      res.end();
    });
  }
  ...
  operator()(Func&& f) {
    ...
    handler_ = ([f = std::forward<Func>(f)](const request&, response& res) { f(res); });
  }
 ...
 public:
  ...
  void handle_upgrade(const request& req, response&, SocketAdaptor&& adaptor) override {
    ...
    new crow::websocket::Connection<SocketAdaptor, App>(
        req, std::forward<SocketAdaptor>(adaptor), app_, max_payload_, subprotocols_, open_handler_, message_handler_,
        close_handler_, error_handler_, accept_handler_, mirror_protocols_);
  }
#ifdef CROW_ENABLE_SSL
  void handle_upgrade(const request& req, response&, SSLAdaptor&& adaptor) override {
    new crow::websocket::Connection<SSLAdaptor, App>(
        req, std::forward<SSLAdaptor>(adaptor), app_, max_payload_, subprotocols_, open_handler_, message_handler_,
        close_handler_, error_handler_, accept_handler_, mirror_protocols_);
  }
#endif
...
class DynamicRule : public BaseRule, public RuleParameterTraits<DynamicRule> {
 public:
  ...
  void validate() override {
    if (!erased_handler_) {
      throw std::runtime_error(name_ + (!name_.empty() ? ": " : "") + "no handler for url " +
                               rule_);
    }
  }
...
#endif
  std::function<void(request&, response&, const routing_params&)> wrap(
    ...
    if (!black_magic::is_parameter_tag_compatible(
      ...
      throw std::runtime_error("route_dynamic: Handler type is mismatched with URL parameters: " +
                               rule_);
    }
...
class TaggedRule : public BaseRule, public RuleParameterTraits<TaggedRule<Args...>> {
 public:
  ...
  template <typename Func>
  void operator()(Func&& f) {
    handler_ = ([f = std::forward<Func>(f)](request& req, response& res, Args... args) {
...
constexpr const uint16_t RULE_SPECIAL_REDIRECT_SLASH{1};
...
class Trie {
 public:
  struct Node {
    uint16_t rule_index{};
    // Assign the index to the maximum 32 unsigned integer value by default so that any other number
    // (specifically 0) is a valid BP id.
    ...
    bool IsSimpleNode() const {
      return !rule_index && blueprint_index == INVALID_BP_ID && children.size() < 2 &&
             param == ParamType::MAX &&
             std::all_of(
 ...
 public:
  void debug_print() {
    CROW_LOG_DEBUG << "└➙ ROOT";
    for (const auto& child : head_.children) debug_node_print(child, 1);
  }
  ...
  routing_handle_result find(const std::string& req_url, const Node& node, unsigned pos = 0,
    ...
    // if the function was called on a node at the end of the string (the last recursion), return
    // the nodes rule index, and whatever params were passed to the function
    if (pos == req_url.size()) {
      auto result = routing_handle_result{node.rule_index, *blueprints, *params};
      found_BP = *blueprints;
      return result;
    }
        ...
        if (child.param == ParamType::INT) {
          ...
          if ((c >= '0' && c <= '9') || c == '+' || c == '-') {
            ...
            if (errno != ERANGE && eptr != req_url.data() + pos) {
              ...
              size_t old_size = blueprints->size();
    ...
    if (!found_fragment) found_BP = *blueprints;

    return routing_handle_result{found, found_BP,
                                 match_params};  // Called after all the recursions have been done
  }

  routing_handle_result find(const std::string& req_url) const { return find(req_url, head_); }

  // This functions assumes any blueprint info passed is valid
  void add(const std::string& url, uint16_t rule_index, unsigned bp_prefix_length = 0,
           uint16_t blueprint_index = INVALID_BP_ID) {
    auto idx = &head_;
    ...
    for (unsigned i = 0; i < url.size(); i++) {
      ...
      if (c == '<') {
        ...
        if (!piece_found) {
          auto new_node_idx = &idx->add_child_node();
          new_node_idx->key = c;
          // The assumption here is that you'd only need to add a blueprint index if the tree didn't
          // have the BP prefix.
    ...
    if (idx->rule_index) throw std::runtime_error("handler already exists for " + url);
...
/// A blueprint can be considered a smaller section of a Crow app, specifically where the router is
/// concerned.

///
/// You can use blueprints to assign a common prefix to rules' prefix, set custom static and
/// template folders, and set a custom catchall route. You can also assign nest blueprints for
...
*/
  Blueprint(Blueprint&& value) noexcept { *this = std::move(value); }
  ...
  void register_blueprint(Blueprint& blueprint) {
    if (blueprints_.empty() ||
        std::find(blueprints_.begin(), blueprints_.end(), &blueprint) == blueprints_.end()) {
...
class Router {
 public:
  ...
  void internal_add_rule_object(const std::string& rule, BaseRule* ruleObject,
    ...
    ruleObject->foreach_method([&](int method) {
      ...
      per_methods_[method].trie.add(
          ...
          BP_index != INVALID_BP_ID ? blueprints[BP_index]->prefix().length() : 0, BP_index);

      // directory case:
      //   request to '/about' url matches '/about/' rule
      if (has_trailing_slash) {
        const uint16_t redirect_index = static_cast<uint16_t>(RULE_SPECIAL_REDIRECT_SLASH);
        const unsigned prefix_len =
            BP_index != INVALID_BP_ID ? blueprints[BP_index]->prefix().length() : 0;
        const uint16_t bp_id = BP_index;
        per_methods_[method].trie.add(rule_without_trailing_slash, redirect_index,
                                      prefix_len, bp_id);
      }
    });
  ...
  void get_recursive_child_methods(Blueprint* blueprint, std::vector<HTTPMethod>& methods) {
    // we only need to deal with children if the blueprint has absolutely no methods (meaning its
    // index won't be added to the trie)
  ...
  void validate_bp() {
    // Take all the routes from the registered blueprints and add them to `all_rules_` to be
    // processed.
    ...
    for (unsigned i = 0; i < blueprints.size(); i++) {
      ...
      if (blueprint->static_dir_ == "" && blueprint->all_rules_.empty()) {
        ...
        for (HTTPMethod x : methods) {
          ...
          per_methods_[method_index].trie.add(blueprint->prefix(), 0, blueprint->prefix().length(),
                                              method_index);
        }
      }
  ...
  // TODO: maybe add actual_method for websockets once Crow's HTTP verb handling
  // is refactored.
  ...
  void handle_upgrade(const request& req, response& res, Adaptor&& adaptor) {
    ...
    if (rule_index >= rules.size()) throw std::runtime_error("Trie internal structure corrupted!");
    ...
    rules[rule_index]->handle_upgrade(req, res, std::forward<Adaptor>(adaptor));
    if (!res.is_completed()) {
  ...
  void get_found_bp(std::vector<uint16_t>& bp_i, std::vector<Blueprint*>& blueprints,
    ...
    // 3. The next blueprint we're adding has a prefix that starts the same as the already added
    // blueprint + a slash (the rest is irrelevant).
    //
    // This is done to prevent a blueprint that has a prefix of "bp_prefix2" to be assumed as a
    // child of one that has "bp_prefix".
    //
    // If any of the assertions is untrue, we delete the last item added, and continue using the
    // blueprint list of the blueprint found before, the topmost being the router's list
  ...
  /// Is used to handle errors, you insert the error code, found route, request, and response. and
  /// it'll either call the appropriate catchall route (considering the blueprint system) and send
  /// you a status string (which is mainly used for debug messages), or just set the response code
  /// to the proper error code.
  std::string get_error(unsigned short code, routing_handle_result& found, const request& req,
                        response& res) {
    res.code = code;
    std::vector<Blueprint*> bps_found;
    get_found_bp(found.blueprint_indices, blueprints_, bps_found);
    for (size_t i = bps_found.size(); i-- > 0;) {
      ...
      if (bps_found[i]->catchall_rule().has_handler()) {
        ...
        return std::string("Redirected to Blueprint \"" + bps_found[i]->prefix() +
                           "\" Catchall rule");
#else
        return std::string();
#endif
  ...
  std::unique_ptr<routing_handle_result> handle_initial(request& req, response& res) {
    ...
    std::unique_ptr<routing_handle_result> found{new routing_handle_result(
        ...
        HTTPMethod::InternalMethodCount)};  // This is always returned to avoid a null pointer
                                            // dereference.

    // NOTE(EDev): This most likely will never run since the parser should handle this situation and
    // close the connection before it gets here.
    if (CROW_UNLIKELY(req.method >= HTTPMethod::InternalMethodCount))
      ...
      *found = per_methods_[static_cast<int>(method_actual)].trie.find(req.url);
      ...
      if (!found->rule_index) {
        ...
        *found = per_methods_[static_cast<int>(method_actual)].trie.find(req.url);
        if (!found->rule_index)  // If a route is still not found, return a 404 without executing
                                 // the rest of the HEAD specific code.
        {
          CROW_LOG_DEBUG << "Cannot match rules " << req.url;
          res = response(404);  // TODO(EDev): Should this redirect to catchall?
                               // Left as-is to mimic Crow's default behaviour.
                               // Behaviour kept for compatibility with older
                               // routing logic.
          res.end();
          return found;
        }
      ...
      if (req.url == "/*") {
        for (int i = 0; i < static_cast<int>(HTTPMethod::InternalMethodCount); i++) {
          if (static_cast<int>(HTTPMethod::Head) == i) continue;  // HEAD is always allowed
    ...
    } else  // Every request that isn't a HEAD or OPTIONS request
    {
      *found = per_methods_[static_cast<int>(method_actual)].trie.find(req.url);
      ...
      // from above (after removing the return statement) to be checked on whether
      // they actually point to a route. Deferred until routing refactor.
      if (!found->rule_index) {
        for (auto& per_method : per_methods_) {
  ...
  handle_rule(BaseRule& rule, crow::request& req, crow::response& res,
    ...
    if (!rule.mw_indices_.empty()) {
      ...
      detail::middleware_call_helper<decltype(crit_fwd), 0, typename App::context_t,
                                     typename App::mw_container_t>(crit_fwd, container, req, res,
                                                                   ctx);
      ...
      res.complete_request_handler_ = [&rule, &ctx, &container, &req, &res,
        ...
        detail::after_handlers_call_helper<decltype(crit_bwd),
  ...
  std::function<void(crow::response&)>& exception_handler() { return exception_handler_; }

  static void default_exception_handler(response& res) {
    // any uncaught exceptions become 500s
    res = response(500);

    CROW_LOG_ERROR << "An uncaught exception occurred.";
  }
