...
class Connection : public connection {
 public:
  ...
  Connection(
      const crow::request& req, Adaptor&& adaptor, Handler* handler, uint64_t max_payload,
      ...
      std::function<bool(const crow::request&, void**)> accept_handler, bool mirror_protocols)
    ...
    if (!utility::string_equals(req.get_header_value("upgrade"), "websocket")) {
      {
        auto ec = adaptor_.close();
        if (ec)
          CROW_LOG_ERROR << "Close failed: " << ec.message();
      }
    ...
    if (!subprotocols.empty() || !requested_subprotocols_header.empty()) {
      ...
      auto subprotocol =
          utility::find_first_of(subprotocols.begin(), subprotocols.end(),
    ...
    std::string magic =
        req.get_header_value("Sec-WebSocket-Key") + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
    sha1::SHA1 s;
  ...
  void close(std::string const& msg, uint16_t status_code) override {
    dispatch([this, msg, status_code]() mutable {
      ...
      if (has_recv_close_ && !is_close_handler_called_) {
        ...
        if (close_handler_) close_handler_(*this, msg, status_code);
      }
 ...
 protected:
  ...
  std::string build_header(int opcode, size_t size) {
    ...
    if (size < 126) {
      ...
      *reinterpret_cast<uint64_t*>(buf + 2) =
          ((1 == htonl(1)) ? static_cast<uint64_t>(size)
    ...
    switch (state_) {
      case WebSocketReadState::MiniHeader: {
        ...
        adaptor_.socket().async_read_some(
            asio::buffer(&mini_header_, 2), [this](const error_code& ec, std::size_t
...

            {
              ...
              if (!ec) {
                if ((mini_header_ & 0x80) == 0x80)
                  has_mask_ = true;
                else  // if the websocket specification is enforced and the message isn't masked,
                ...
                do_read();
                  ...
                  if (error_handler_) error_handler_(*this, ec.message());
                  check_destroy();
                }
            });
      } break;
        ...
        asio::async_read(
            ...
            [this](const error_code& ec, std::size_t
              ...
              remaining_length_ =
                  ((1 == ntohl(1))
                       ? (remaining_length_)
                       : (static_cast<uint64_t>(ntohl((remaining_length_) & 0xFFFFFFFF)) << 32) |
      ...
      case WebSocketReadState::Mask:
        if (remaining_length_ > max_payload_bytes_) {
          ...
          if (error_handler_) error_handler_(*this, "Message length exceeds maximum payload.");
  ...
  ///
  /// Unmasks the fragment, checks the opcode, merges fragments into 1 message body, and calls the
  /// appropriate handler.
      ...
      case 0x8:  // Close
      {
        ...
        if (!has_sent_close_) {
          ...
          if (!is_close_handler_called_) {
            if (close_handler_) close_handler_(*this, fragment_.substr(message_start), status_code);
            is_close_handler_called_ = true;
          }
          {
            auto sec = adaptor_.shutdown_readwrite();
      ...
      asio::async_write(adaptor_.socket(), buffers,
                        [&, watch](const error_code& ec, std::size_t /*bytes_transferred*/) {
                          if (!ec && !close_connection_) {
                            ...
                            if (has_sent_close_) close_connection_ = true;
                          } else {
  ...
  void check_destroy(websocket::CloseStatusCode code = CloseStatusCode::ClosedAbnormally) {
    // Note that if the close handler was not yet called at this point we did not receive a close
    // packet (or send one) and thus we use ClosedAbnormally unless instructed otherwise
    if (!is_close_handler_called_)
 ...
 private:
  ...
  std::function<void(crow::websocket::connection&, const std::string&, uint16_t status_code)>
      close_handler_;
