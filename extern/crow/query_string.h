...
// Enable sorting of the parsed key/value pairs when requested.
#ifdef CROW_ENABLE_QS_SORTING
#define _qsSORTING
#endif
...
#ifdef _qsSORTING
    auto cmp = [&](const char* entry, const char* k) {
        size_t entry_len = strcspn(entry, "=");
        size_t min_len   = entry_len < key_len ? entry_len : key_len;
        int r            = qs_strncmp(k, entry, min_len);
        if (r == 0)
        {
            if (key_len == entry_len)
                return 0;
            return key_len < entry_len ? -1 : 1;
        }
        return r;
    };

    const char* const* begin = qs_kv;
    const char* const* end   = qs_kv + qs_kv_size;
    auto it = std::lower_bound(begin, end, key, [&](const char* entry, const char* k) {
        return cmp(entry, k) < 0;
    });
    for (; it != end && cmp(*it, key) == 0; ++it)
    {
        skip = strcspn(*it, "=");
        if ((*it)[skip] == '=')
            skip++;
        if (nth == 0)
            return const_cast<char*>(*it + skip);
        --nth;
    }
#else  // _qsSORTING
...
inline std::unique_ptr<std::pair<std::string, std::string>> qs_dict_name2kv(const char * dict_name, char * const * qs_kv, size_t qs_kv_size, int nth = 0)
{
    ...
    auto cmp = [&](const char* entry, const char* k) {
        size_t entry_len = strcspn(entry, "[");
        size_t len       = entry_len < name_len ? entry_len : name_len;
        int r            = std::strncmp(k, entry, len);
        if (r == 0)
        {
            if (name_len == entry_len)
                return 0;
            return name_len < entry_len ? -1 : 1;
        }
        return r;
    };
    ...
    const char* const* begin = qs_kv;
    const char* const* end   = qs_kv + qs_kv_size;
    auto it = std::lower_bound(begin, end, dict_name, [&](const char* entry, const char* k) {
        return cmp(entry, k) < 0;
    });
    for (; it != end && cmp(*it, dict_name) == 0; ++it)
    {
        skip_to_eq         = strcspn(*it, "=");
        if ((*it)[skip_to_eq] == '=')
            skip_to_eq++;
        skip_to_brace_open  = strcspn(*it, "[");
        if ((*it)[skip_to_brace_open] == '[')
            skip_to_brace_open++;
        skip_to_brace_close = strcspn(*it, "]");
        ...
        if (skip_to_brace_open <= skip_to_brace_close &&
            ...
            nth == 0)
        {
            auto key   = std::string(*it + skip_to_brace_open, skip_to_brace_close - skip_to_brace_open);
            auto value = std::string(*it + skip_to_eq);
...
inline char * qs_scanvalue(const char * key, const char * qs, char * val, size_t val_len)
{
    ...
    if ( qs[0] == '=' )
    {
        qs++;
        i = strcspn(qs, "&=#");
        size_t copy_len = (val_len - 1) < (i + 1) ? (val_len - 1) : (i + 1);
        (void)std::snprintf(val, val_len, "%.*s", static_cast<int>(copy_len), qs);
        qs_decode(val);
    }
    else
    {
...
namespace crow
{
    ...
    public:
          ...
          url_(std::move(params))
        {
            ...
            size_t count = qs_parse(url_.data(), key_value_pairs_.data(), MAX_KEY_VALUE_PAIRS_COUNT, url);
