 ...
 * dff604db203986e532e5a679bafd0e7382c6bdd9 (Might be useful to actually add [upgrade requests with
 * a body]) e01811e7f4894d7f0f7f4bd8492cccec6f6b4038 (related to above)
    ...
    switch (parser->state) {
      ...
      case s_start_req:
      {
        ...
        switch (ch) {
          ...
          case 'L': parser->method = (unsigned)
          HTTPMethod::Lock;                                                             break;
          case 'M': parser->method = (unsigned)
          HTTPMethod::MkCol;                                                            break;
          case 'N': parser->method = (unsigned)HTTPMethod::Notify;                                                           break;
          case 'O': parser->method = (unsigned)HTTPMethod::Options;                                                          break;
          case 'P': parser->method = (unsigned)
          HTTPMethod::Post;                                                             break;
          case 'R': parser->method = (unsigned)
          HTTPMethod::Report;                                                           break;
          case 'S': parser->method = (unsigned)
          HTTPMethod::Subscribe;                                                        break;
          case 'T': parser->method = (unsigned)HTTPMethod::Trace;                                                            break;
          case 'U': parser->method = (unsigned)
          HTTPMethod::Unlock;                                                           break;
          default:
            CROW_SET_ERRNO(CHPE_INVALID_METHOD);
            goto error;
        }
...
*/
          ...
          switch (h_state) {
            ...
            case h_matching_transfer_encoding_token_start:
              ...
              if ('c' == c) {
                 ...
                 * `STRICT_TOKEN`. Keeping original behaviour for parity with
                 * upstream http_parser.
                 */
      ...
      case s_chunk_size_start:
      {
        ...
        parser->content_length = static_cast<uint64_t>(static_cast<unsigned char>(unhex_val));
      ...
      {
        assert(parser->flags & F_CHUNKED);
        /* just ignore this for now. TODO check for overflow when proper bounds
