...
struct HTTPParser : public http_parser {
  ...
  HTTPParser(Handler* handler) : http_parser(), handler_(handler) { http_parser_init(this); }
  ...
  inline void set_connection_parameters() {
    ...
    // NOTE(EDev): it seems that the problem is with crow's policy on closing the connection for
    // HTTP_VERSION < 1.0, the behaviour for that in crow is "don't close the connection, but don't
    // send a keep-alive either"
    ...
    // HTTP1.1 = only close if close header exists, HTTP1.0 = always close unless keep_alive header
    // exists, HTTP?.?= never close
    req.close_connection =
        (http_major == 1 && http_minor == 0)
            ? ((flags & F_CONNECTION_KEEP_ALIVE) ? false : true)
            : ((http_major == 1 && http_minor == 1) ? ((flags & F_CONNECTION_CLOSE) ? true : false)
                                                    : false);
  ...
  ///
  /// Data parsed is put directly into this object as soon as the related callback returns. (e.g.
  /// the request will have the cooorect method as soon as on_method() returns)
  request req;
