...
/// Finds and returns the header with the specified key. (returns an empty header if nothing is
/// found)
...
struct part_view {
  mph_view_map headers;   ///< (optional) The first part before the data, Contains information
                          ///< regarding the type of data and encoding
...
struct message_view {
  ...
  mp_view_map part_map;  ///< The individual parts of the message, organized in a map with the
                         ///< `name` header parameter being the key
  ...
  explicit message_view(const request& req)
      : headers(req.headers), boundary(get_boundary(get_header_value("Content-Type"))) {
    parse_body(req.body);
  }
 ...
 private:
  ...
  void parse_body(std::string_view body) {
    ...
    // TODO(EDev): Exit on error if delimiter is missing. Currently the parser
    // falls back to ignoring the remainder of the body.
    while (body != (crlf)) {
