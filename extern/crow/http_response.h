...
#if defined(_MSC_VER)
#ifndef CROW_CRT_NONSTDC_NAMES
#define CROW_CRT_NONSTDC_NAMES 1
#endif
#define SEP_CRT_INTERNAL_NONSTDC_NAMES CROW_CRT_NONSTDC_NAMES
#endif
...
struct response {
  ...
  bool compressed = true;  ///< If compression is enabled and this is false, the individual response
                           ///< will not be compressed.
#endif
  bool skip_body = false;  ///< Whether this is a response to a HEAD request.
  bool manual_length_header =
      false;  ///< Whether Crow should automatically add a "Content-Length" header.
  ...
  static std::string get_mime_type(const std::string& contentType) {
    ...
    if (mimeTypeIterator != mime_types.end()) {
      ...
      CROW_LOG_WARNING << "Unable to interpret mime type for content type '" << contentType
  ...
  response(response&& r) noexcept { *this = std::move(r); }
  ...
  /// This constains metadata (coming from the `stat` command) related to any static files
