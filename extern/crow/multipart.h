...
struct header {
  std::string
      value;  ///< The first part of the header, usually `Content-Type` or `Content-Disposition`
...
/// Find and return the value object associated with the key. (returns an empty class if nothing is
/// found)
...
struct part {
  mph_map headers;   ///< (optional) The first part before the data, Contains information regarding
                     ///< the type of data and encoding
...
struct message : public returnable {
  ...
  mp_map part_map;  ///< The individual parts of the message, organized in a map with the `name`
  ...
  message(const ci_map& headers_, const std::string& boundary_, const std::vector<part>& sections)
      : returnable("multipart/form-data; boundary=CROW-BOUNDARY"),
        ...
        boundary(boundary_),
        parts(sections) {
    if (!boundary.empty()) content_type = "multipart/form-data; boundary=" + boundary;
    for (auto& item : parts) {
      part_map.emplace(
  ...
  explicit message(const request& req)
    ...
    if (!boundary.empty()) {
      ...
      (void)fprintf(stderr, "Empty boundary in multipart message\n");
      assert(false && "Empty boundary in multipart message");
    }
  }
 ...
 private:
  ...
  void parse_body(std::string body) {
    ...
    // TODO(EDev): Exit on error rather than asserting once proper error
    // propagation is defined.
    while (body != (crlf)) {
      size_t found = body.find(delimiter);
      if (found == std::string::npos) {
        // did not find delimiter; probably an ill-formed body
        (void)fprintf(stderr,
                "Unable to find delimiter in multipart message. Probably ill-formed body\n");
        assert(false && "Invalid multipart delimiter");
      }
      std::string section = body.substr(0, found);

      // +2 is the CRLF.
      // We don't check it and delete it so that the same delimiter can be used for The last
      // delimiter (--delimiter--CRLF).
      body.erase(0, found + delimiter.length() + 2);
      if (!section.empty()) {
        part parsed_section(parse_section(section));
        part_map.emplace((get_header_object(parsed_section.headers, "Content-Disposition")
                              .params.find("name")
                              ->second),
                         parsed_section);
