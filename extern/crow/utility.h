...
// useful when the project switches to C++20.
...
constexpr unsigned requires_in_range(unsigned i, unsigned len) {
  if (i >= len) {
    sep::crow::error::log(sep::crow::error::Code::OutOfRange, "index out of range");
    return len ? len - 1 : 0;
  }
  return i;
}
...
static inline unsigned find_closing_tag_runtime(const char* s, unsigned p) {
  if (s[p] == 0) {
    sep::crow::error::log(sep::crow::error::Code::InvalidParameter, "unmatched tag <");
    return p;
  }
  return s[p] == '>' ? p : find_closing_tag_runtime(s, p + 1);
}
  ...
  return s[p] == 0 ? 0
         : s[p] == '<'
             ? (std::strncmp(s + p, "<int>", 5) == 0
                ...
                : (std::strncmp(s + p, "<str>", 5) == 0 || std::strncmp(s + p, "<string>", 8) == 0)
                ...
                : std::strncmp(s + p, "<path>", 6) == 0
                    ...
                    : (sep::crow::error::log(sep::crow::error::Code::InvalidParameter, "invalid parameter type"), 0))
             : get_parameter_tag_runtime(s, p + 1);
}
#ifndef CROW_MSVC_WORKAROUND
constexpr uint64_t get_parameter_tag(const_str s, unsigned p = 0) {
  return p == s.size() ? 0
         : s[p] == '<' ? (is_int(s, p)     ? get_parameter_tag(s, find_closing_tag(s, p)) * 6 + 1
...
struct CallHelper<F, S<Args...>> {
  ...
  static char test_helper(int);

  template <typename...>
  static int test_helper(...);

  static constexpr bool value = sizeof(test_helper<F, Args...>(0)) == sizeof(char);
};
...
inline static std::string base64encode_urlsafe(std::string data, size_t size) {
  return base64encode((const unsigned char*)data.c_str(), size,
                      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_");
}

inline static std::string base64decode(const char* data, size_t size) {
  // We accept both regular and url encoding here, as there does not seem to be any downside to
  // that. If we want to distinguish that we should use +/ for non-url and -_ for url.
  ...
  if (size % 4 == 2)  // missing last 2 characters
    size = (size / 4 * 3) +
           1;  // Not subtracting extra characters because they're truncated in int division
  else if (size % 4 == 3)  // missing last character
    size = (size / 4 * 3) +
           2;  // Not subtracting extra characters because they're truncated in int division

  // Padded
  else if (size >= 2 && data[size - 2] == '=')  // padded with '=='
    size = (size / 4 * 3) -
           2;  // == padding means the last block only has 1 character instead of 3, hence the '-2'
  else if (size >= 1 && data[size - 1] == '=')  // padded with '='
    size = (size / 4 * 3) -
           1;  // = padding means the last block only has 2 character instead of 3, hence the '-1'
  ...
  while (size >= 3) {
    // dec_char1 = (char1 shifted 2 bits to the left) OR ((char2 AND 00110000) shifted 4 bits to the
    // right))
    odd = key(*data++);
    even = key(*data++);
    *it++ = (odd << 2) | ((even & 0x30) >> 4);
    // dec_char2 = ((char2 AND 00001111) shifted 4 bits left) OR ((char3 AND 00111100) shifted 2
    // bits right))
    odd = key(*data++);
  ...
  if (size == 2) {
    // d_char1 = (char1 shifted 2 bits to the left) OR ((char2 AND 00110000) shifted 4 bits to the
    // right))
    odd = key(*data++);
    even = key(*data++);
    *it++ = (odd << 2) | ((even & 0x30) >> 4);
    // d_char2 = ((char2 AND 00001111) shifted 4 bits left) OR ((char3 AND 00111100) shifted 2 bits
    // right))
    odd = key(*data++);
    *it++ = ((even & 0x0F) << 4) | ((odd & 0x3C) >> 2);
  } else if (size == 1) {
...
inline static void sanitize_filename(std::string& data, char replacement = '_') {
  ...
  // Check for special device names. The Windows behavior is really odd here, it will consider both
  // AUX and AUX.txt a special device. Thus we search for the string (case-insensitive), and then
  // check if the string ends or if is has a dangerous follow up character (.:\/)
  auto sanitizeSpecialFile = [](std::string& source, unsigned ofs, const char* pattern,
                                bool includeNumber, char replacement_) {
    unsigned i = ofs;
    ...
    if ((i >= len) || (source[i] == '.') || (source[i] == ':') || (source[i] == '/') ||
        (source[i] == '\\')) {
  ...
  for (unsigned i = 0; i < data.length(); ++i) {
    ...
    if ((c < ' ') || ((c >= 0x80) && (c <= 0x9F)) || (c == '?') || (c == '<') || (c == '>') ||
        (c == ':') || (c == '*') || (c == '|') || (c == '\"')) {
      data[i] = replacement;
    } else if ((c == '/') || (c == '\\')) {
      if (CROW_UNLIKELY(i == 0))  // Prevent Unix Absolute Paths (Windows Absolute Paths are
                                  // prevented with `(c == ':')`)
      {
        data[i] = replacement;
      } else {
 ...
 */
inline static bool string_equals(const std::string_view l, const std::string_view r,
