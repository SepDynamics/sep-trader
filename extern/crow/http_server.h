...
class Server {
 public:
  Server(Handler* handler, const tcp::endpoint& endpoint,
         ...
         uint8_t timeout = 5, typename Adaptor::context* adaptor_ctx = nullptr)
  ...
  void run() {
...
#else
                            (void)gmtime_r(&last_time_t, &my_tm);
#endif
          date_str.resize(100);
          size_t date_str_sz = strftime(date_str.data(), 99, "%a, %d %b %Y %H:%M:%S GMT", &my_tm);
    ...
    CROW_LOG_INFO << server_name_ << " server is running at "
                  ...
                  << acceptor_.local_endpoint().port() << " using " << concurrency_ << " threads";
    CROW_LOG_INFO << "Call `app.loglevel(crow::LogLevel::Warning)` to hide Info level logs.";

    signals_.async_wait([&](const error_code& /*error*/, int /*signal_number*/) { stop(); });

    while (worker_thread_count != init_count) std::this_thread::yield();
 ...
 private:
  uint16_t pick_io_context_idx() {
    ...
    // Scan all contexts and pick the one with the smallest queue length.
    for (size_t i = 1; i < task_queue_length_pool_.size(); i++)
    {
  ...
  void do_accept() {
    if (!shutting_down_) {
      ...
      CROW_LOG_DEBUG << &ic << " {" << context_idx
                     << "} queue length: " << task_queue_length_pool_[context_idx];

      auto p = std::make_shared<Connection<Adaptor, Handler, Middlewares...>>(
          ic, handler_, server_name_, middlewares_, get_cached_date_str_pool_[context_idx],
          *task_timer_pool_[context_idx], adaptor_ctx_, task_queue_length_pool_[context_idx]);

      acceptor_.async_accept(p->socket(), [this, p, &ic, context_idx](error_code ec) {
        if (!ec) {
