...
class Crow {
 public:
  ...
  void handle_upgrade(const request& req, response& res, Adaptor&& adaptor) {
    router_.handle_upgrade(req, res, std::forward<Adaptor>(adaptor));
  }
  ...
  void handle_full(request& req, response& res) {
    ...
    if (found->rule_index) handle(req, res, found);
  }

  /// \brief Create a dynamic route using a rule (**Use CROW_ROUTE instead**)
  DynamicRule& route_dynamic(const std::string& rule) { return router_.new_rule_dynamic(rule); }

  /// \brief Create a route using a rule (**Use CROW_ROUTE instead**)
  template <uint64_t Tag>
  auto route(const std::string& rule) ->
      typename std::invoke_result<decltype(&Router::new_rule_tagged<Tag>), Router,
                                  const std::string&>::type {
    return router_.new_rule_tagged<Tag>(rule);
  }

  /// \brief Create a route for any requests without a proper route (**Use CROW_CATCHALL_ROUTE
  /// instead**)
  ...
  /// \brief Set the server name included in the 'Server' HTTP response header. If set to an empty
  /// string, the header will be omitted by default.
  ...
  /// \brief Set the response body size (in bytes) beyond which Crow automatically streams responses
  /// (Default is 1MiB)
  ///
  /// Any streamed response is unaffected by Crow's timer, and therefore won't timeout before a
  ...
  /// \brief Set the function to call to handle uncaught exceptions generated in routes (Default
  /// generates error 500).
  ///
  /// The function must have the following signature: void(crow::response&).
  /// It must set the response passed in argument to the function, which will be sent back to the
  /// client. See Router::default_exception_handler() for the default implementation.
  ...
  std::function<void(crow::response&)>& exception_handler() { return router_.exception_handler(); }
  ...
  void run() {
    ...
    error_code ec;
#ifdef CROW_USE_BOOST
    boost::asio::ip::address addr = boost::asio::ip::make_address(bindaddr_, ec);
#else
    asio::ip::address addr = asio::ip::make_address(bindaddr_, ec);
#endif
    if (ec) {
      CROW_LOG_ERROR << ec.message() << " - Can not create valid ip address from string: \""
                     << bindaddr_ << "\"";
      return;
    }
    ...
    if (ssl_used_) {
      ...
      ssl_server_ = std::make_unique<ssl_server_t>(this, endpoint, server_name_,
                                                  &middlewares_, concurrency_,
                                                  timeout_, &ssl_context_);
...
#endif
    {
      server_ = std::make_unique<server_t>(this, endpoint, server_name_,
                                           &middlewares_, concurrency_, timeout_,
                                           nullptr);
...
#endif
    {
      // TODO(EDev): Move these 6 lines to a method in http_server. The refactor
      // is tracked in docs/tasks.md and left here to avoid larger changes until
      // the HTTP server abstraction is finalized.
  ...
  std::cv_status wait_for_server_start(
 ...
 private:
  ...
  std::tuple<Middlewares...> make_middleware_tuple(Ts&&... ts) {
    ...
    return std::make_tuple(
        std::forward<Middlewares>(black_magic::tuple_extract<Middlewares, decltype(fwd)>(fwd))...);
  }
