...
struct Action {
  ...
  Action(char tag_char_, ActionType t_, size_t start_, size_t end_, size_t pos_ = 0)
      : has_end_match(false),
        tag_char(tag_char_),
        start(static_cast<int>(start_)),
        end(static_cast<int>(end_)),
        pos(static_cast<int>(pos_)),
        t(t_) {}
...
class template_t {
 ...
 private:
  ...
  auto find_context(const std::string& name, const std::vector<const context*>& stack,
    ...
    if (dotPosition == std::string::npos) {
      ...
      for (std::size_t i = 1; i < dotPositions.size(); ++i)
        names.emplace_back(
  ...
  void render_internal(int actionBegin, int actionEnd, std::vector<const context*>& stack,
    ...
    while (current < actionEnd) {
      ...
      switch (action.t) {
        ...
        case ActionType::Partial: {
          ...
          partial_templ.render_internal(0, partial_templ.fragments_.size() - 1, stack, out,
        ...
        case ActionType::Tag: {
          ...
          switch (ctx.t()) {
            ...
            default:
              (void)fprintf(stderr, "not implemented tag type %d\n", static_cast<int>(ctx.t()));
              assert(false && "not implemented tag type");
          }
        } break;
        case ActionType::ElseBlock: {
        ...
        case ActionType::OpenBlock: {
          ...
          switch (ctx.t()) {
            ...
            default:
              (void)fprintf(stderr,
                            "{{#: not implemented context type: %d\n",
                            static_cast<int>(ctx.t()));
              assert(false && "unsupported context type");
              break;
          }
          break;
        }
        case ActionType::CloseBlock:
          ...
          break;
        default:
          (void)fprintf(stderr, "not implemented %d\n", static_cast<int>(action.t));
          assert(false && "not implemented action type");
      }
      current++;
    }
 ...
 public:
  ...
  /// Apply the values from the context provided and output a returnable template from this mustache
  /// template
 ...
 private:
  void parse() {
    ...
    while (1) {
      ...
      if (endIdx == idx) {
        (void)fprintf(stderr, "mustache error"); assert(false && "mustache error");
      }
          ...
          endIdx++;
          {
            ...
            for (size_t i = idx; i < endIdx; i++) {
              if (body_[i] == ' ') {
                ...
                if (tag_open.empty()) (void)fprintf(stderr, "mustache error"); assert(false && "mustache error");
                if (tag_close.empty()) (void)fprintf(stderr, "mustache error"); assert(false && "mustache error");
    ...
    for (int i = static_cast<int>(actions_.size()) - 2; i >= 0; i--) {
      if (actions_[i].t == ActionType::Tag || actions_[i].t == ActionType::UnescapeTag) continue;
      ...
      for (k = fragment_after.first;
           k < static_cast<int>(body_.size()) && k < fragment_after.second; k++) {
        if (body_[k] != ' ') {
