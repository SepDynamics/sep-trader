...
- Custom `pqxx_time_point_traits` used function-based `has_null()` which was not a constant expression, causing PostgreSQL integration to fail. Replaced it with `constexpr` flags `has_null`/`always_null` and added an explicit `pqxx::nullness` specialization for `std::chrono::system_clock::time_point` to satisfy newer `pqxx` constant-evaluation checks.
