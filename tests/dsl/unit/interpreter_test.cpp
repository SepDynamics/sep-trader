...
protected:
    ...
    void runCodeExpectingError(const std::string& code)
    {
        Parser parser(code);
        auto program = parser.parse();
...
TEST_F(InterpreterTest, ExecutesSimpleAssignment) { runCode("pattern test { x = 42; }"); }
...
TEST_F(InterpreterTest, HandlesEmptyPattern) { runCode("pattern empty { }"); }
...
{
    EXPECT_THROW(
        { runCodeExpectingError("pattern test { x = undefined_variable; }"); }, std::runtime_error);
}

TEST_F(InterpreterTest, HandlesUndefinedFunctions)
{
    EXPECT_THROW(
        { runCodeExpectingError("pattern test { x = unknown_function(5); }"); },
        std::runtime_error);
}
...
TEST_F(InterpreterTest, HandlesWrongArgumentCounts)
{
    ...
    EXPECT_THROW({ runCodeExpectingError("pattern test { x = abs(1, 2); }"); }, std::runtime_error);

    // min() expects 2 arguments
    EXPECT_THROW({ runCodeExpectingError("pattern test { x = min(5); }"); }, std::runtime_error);
}

TEST_F(InterpreterTest, HandlesNegativeSqrt)
{
    EXPECT_THROW({ runCodeExpectingError("pattern test { x = sqrt(-1); }"); }, std::runtime_error);
}

TEST_F(InterpreterTest, ExecutesPrintFunction)
{
    // Test that print function executes without throwing
    EXPECT_NO_THROW({ runCode("pattern test { print(\"Hello\", 42, true); }"); });
}
