...
export const symbolInfo: Record<Symbol, { name: string; minSpread: number; precision: number }> = {
...
export const getSymbolName = (symbol: Symbol) => symbolInfo[symbol]?.name || symbol;
export const getSymbolPrecision = (symbol: Symbol) => symbolInfo[symbol]?.precision || 5;
export const getSymbolMinSpread = (symbol: Symbol) => symbolInfo[symbol]?.minSpread || 0.0001;